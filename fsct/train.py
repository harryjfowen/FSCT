...
class TrainModel:
    ...
    """
    NOTE: PERHAPS SUBSTITUTE WITH PHIL'S PREPROCESSING MODULE FOR CONSISTENCY
    """
    @staticmethod
    def threaded_boxes(point_cloud, box_size, min_points_per_box, max_points_per_box, path, id_offset, point_divisions):
        ...
        while i < pds:
            ...
            box = box[
                np.logical_and(
                    np.logical_and(
                        np.logical_and(box["x"] >= box_centre_mins[i, 0], box["x"] < box_centre_maxes[i, 0]),
                        np.logical_and(box["y"] >= box_centre_mins[i, 1], box["y"] < box_centre_maxes[i, 1]),
                    ),
                    np.logical_and(box["z"] >= box_centre_mins[i, 2], box["z"] < box_centre_maxes[i, 2]),
                )
            ]
            ...
            if box.shape[0] > min_points_per_box:
                if box.shape[0] > max_points_per_box:
                    """
                    COMPUTE GEOMETRIC FEATURES WITHIN POINT CLOUD BOX
                    """
                    features = compute_features(box.astype('double')[['x','y','z']], search_radius=0.05,feature_names=["linearity","surface_variation"], num_threads=1)
                    
                    """
                    STACK REFLECTANCE COLUMN WITH GEOMETRIC FEATURE VALUES
        	        
                    auxilary = np.column_stack((box['scalar_refl'],features))
                    
                    
                    NORMALISE ALL VALUES TO BETWEEN 0 AND 1
        	        """
                    auxilary = preprocessing.minmax_scale(features, feature_range=(0,1))
                    """
                    TAKE THE MAXIMUM VALUE OF ALL FEATURES TO USE AS 1D ARRAY WEIGHTING
                    """
                    weights = np.amax(auxilary,axis=1)
                    """
                    RANDOMLY SUBSAMPLE DATA BUT USING WEIGHTS THAT REPRESENT "LEAVES"
                    """
                    box = box.sample(n=parameters["max_points_per_box"],weights=weights)
    ...
    def preprocess_point_cloud(self, point_cloud, sample_dir):
        ...
        point_cloud_range = point_cloud_max - point_cloud_min
        point_cloud_centre = point_cloud_min + 0.5 * point_cloud_range

        num_boxes_array = np.ceil(point_cloud_range / self.parameters["sample_box_size_m"])
        box_centres = np.vstack(np.meshgrid(*get_box_centre_list(point_cloud_min, num_boxes_array))).reshape(3, -1).T
    ...
    def run_training(self):
        ...
        train_dataset = TrainingDataset(
            ...
            augmentation=parameters["perform_data_augmentation"],
        )
        ...
        model = Net(num_classes=2).to(self.device)
...
if __name__ == "__main__":
    parameters = dict(
        ...
        model_filename="hjfoV1.pth",
        sample_box_size_m=np.array([1, 1, 1]),
        sample_box_overlap=[0.1, 0.1, 0.1],

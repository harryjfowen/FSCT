...
pc = pd.concat([pc,features],axis=1)
save_file('/home/harryowen/Desktop/xyz-features.ply',pc, additional_fields=extra_cols+['refl'])

## Split the cloud based on reflectance threshold
cutoff = np.percentile(pc['refl'].values, 90)
refl_bool = np.any(pc[['refl']] > cutoff, axis=1)

##  First lets deal with the leaves 
tmp = pc[~refl_bool]
is_wood=np.any(tmp[extra_cols] > 0.80, axis=1)

#clustering 
leaf_clusters = euclidean_clustering(tmp[~is_wood],16,3)
labels=leaf_clusters[['id']].values.astype(int)
leaf_cleaned = cluster_filter(leaf_clusters[['x','y','z']].values,labels,0.66,direction='negative')

##writeout 
leaf = leaf_clusters[leaf_cleaned][['x','y','z']] 
save_file('/home/harryowen/Desktop/leaf.ply',leaf, additional_fields=[])

## NOW WOOD
#rejected wood from clustering
rejected_leaf = leaf_clusters[~leaf_cleaned]

labels=rejected_leaf[['id']].values.astype(int)
wood_cleaned1 = cluster_filter(rejected_leaf[['x','y','z']].values,labels,0.75,direction='positive')
wood = rejected_leaf[wood_cleaned1]

wood2 = pc[refl_bool]

wood_final = pd.concat([wood[['x','y','z']],wood2[['x','y','z']]])

##clean up wood
wood_final = wood_final.iloc[denoise(wood_final, 10, 1)]
wood_clusters = euclidean_clustering(wood_final,16,10)
labels=wood_clusters[['id']].values.astype(int)
wood_final_clean = cluster_filter(wood_final[['x','y','z']].values,labels,0.66,direction='positive')
wood_final = wood_clusters[wood_final_clean]
save_file('/home/harryowen/Desktop/wood_final.ply',wood_final[['x','y','z']], additional_fields=[])
...
wood_clusters = euclidean_clustering(pc.loc[pc['label']==0],16,3)
##filter clusters based on shape
labels=wood_clusters[['id']].values.astype(int)
x = cluster_filter(wood_clusters[['x','y','z']].values,labels,0.66,direction='positive',cols=extra_cols)
